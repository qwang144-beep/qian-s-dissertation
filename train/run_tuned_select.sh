#!/bin/bash
#SBATCH --job-name=tunedSEL
#SBATCH --partition=gpu
#SBATCH --qos=gpu
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
#SBATCH --time=12:00:00
#SBATCH --output=tunedSEL_%j.out
#SBATCH --error=tunedSEL_%j.err
...
$PY src/generate_metrical.py \
  --manifest prompts_300.jsonl --skeletons skeletons_300.jsonl \
  --out-dir metrical_300_tunedSEL --model /mnt/parscratch/users/smp23qw/qwen_metrical_sft \
  --n-candidates 8 --k-context 3 --temperature 0.9 \
  --fix-rhyme --tail-window 3 \
  --w-count 3.0 --w-rhyme 1.0 --w-vocab 0.7 --w-rep 0.8 \
  --dump-candidates cand_dump_tunedSEL.jsonl
